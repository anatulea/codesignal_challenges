...
def isTestSolvable(ids, k):
    # converts id in string 23742 = ['2','3','7','4', '2'] ---> str(id)
    # iterates over the list of string digits
    # converts them back to integers ---> int(digit)
    # and sums them all  ----> sum()
    ...
    # digitSum = lambda x: sum([int(y) for y in list(str(x))])
    # digitSum = lambda questionId: sum([ord(str(questionId)[i])-ord('0') for i in range(len(str(questionId))) ])
